...
\section{Including Deleterious Mutations and a Fitness Function}\label{sec:del}
  ...
  \begin{minipage}{0.5\textwidth}
    ...
    \includegraphics[width=0.95\textwidth]{figure_2x2mutation01}%
      \captionof{figure}{The transfer function for $A = \left[ \protect\begin{array}{cc} -3 & 1 \\ 0 & -1 \protect\end{array} \right]$ and all eight $\mu(\Sigma, A_{ij})$ mutations, where $\upsilon = \pm 0.5$}\label{fig:mut}%
  \end{minipage}
...
Further let $\mu(\Sigma, A_{ij})$ be the resulting gene regulatory network following a mutation of matrix element $A_{ij}$ to system $\Sigma$ (which contains matrix $A$).

  \begin{align*}
    \Phi(\mu(\Sigma, A_{ij} \upsilon)) = \exp\left(- \left| \frac{A_{22}-A_{12}}{\lambda_{1} \lambda_{2}} - \frac{k + \upsilon \pda (A_{22}-A_{12})}{\lambda_{1} \lambda_{2} + \text{adj}(A)^{T}_{ij} \upsilon} \right|\right)   .
  \end{align*}
For instance, the fitness of an organism with a change of one of its entries $A_{ij}$ (by $\upsilon$) is listed,
  \begin{align*}
    \Phi(\mu(\Sigma, A_{11})) &= \exp \left(- \left| \frac{A_{22}-A_{12}}{\det(A)} - \frac{A_{22}-A_{12}}{\det(A) + \upsilon A_{22}} \right| \right) \\
    \Phi(\mu(\Sigma, A_{12})) &= \exp \left(- \left| \frac{A_{22}-A_{12}}{\det(A)} - \frac{A_{22}-A_{12} - \upsilon}{\det(A) - \upsilon A_{21}} \right| \right) \\
    \Phi(\mu(\Sigma, A_{21})) &= \exp \left(- \left| \frac{A_{22}-A_{12}}{\det(A)} - \frac{A_{22}-A_{12}}{\det(A) - \upsilon A_{12}} \right| \right) \\
    \Phi(\mu(\Sigma, A_{22})) &= \exp \left(- \left| \frac{A_{22}-A_{12}}{\det(A)} - \frac{A_{22}-A_{12} + \upsilon}{\det(A) + \upsilon A_{11}} \right| \right)
  \end{align*}
...
%  \begin{align*}
%    \frac{\left(A_{22}-A_{12}\right) + \left( \displaystyle\sum_{\jmath = 1}^{\ind_{\omega}} \omega_{\jmath} - \displaystyle\sum_{\jmath = 1}^{\ind_{\varepsilon}} \varepsilon_{\jmath} \right)}{\displaystyle\det(A) + \displaystyle\sum_{\jmath=1}^{\ind_{\omega}} \omega_{\jmath} A_{11} - \sum_{\jmath=1}^{\ind_{\delta}} \delta_{\jmath} A_{12} -  \sum_{\jmath=1}^{\ind_{\varepsilon}} \varepsilon_{\jmath} A_{21} + \sum_{\jmath=1}^{\ind_{\upsilon}} \upsilon_{\jmath} A_{22} + \sum_{\jmath=1}^{\ind_{\omega}} \omega_{\jmath} \sum_{\jmath=1}^{\ind_{\upsilon}} \upsilon_{\jmath} - \sum_{\jmath=1}^{\ind_{\varepsilon}} \varepsilon_{\jmath} \sum_{\jmath=1}^{\ind_{\delta}} \delta_{\jmath} }
%  \end{align*}
