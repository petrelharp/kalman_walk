...
\section{Including Deleterious Mutations and a Fitness Function}
...
\begin{align}\label{eq:fitness}
  \Phi(\bar{\Sigma}) := \left\{ \begin{array}{ll}   e^{- | H(0) - \tilde{H}(0) | = - \left| \frac{k}{\lambda_{1} \lambda_{2}} - \frac{\tilde{k}}{\tilde{\lambda}_{1} \tilde{\lambda}_{2}} \right|}    &\text{if } \lambda_{1,2} < 0 \text{ and } \in \mathbb{R}, \ \det(A) \neq 0 \\
...
Further let $\mu(\Sigma, A_{ij})$ be resulting gene regulatory network following a mutation of matrix element $A_{ij}$ to system $\Sigma$ (which contains matrix $A$).
