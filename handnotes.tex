...
\section*{Evolutionary conservation of phenotype does not entail conservation of the underlying molecular mechanism leading to the rapid formation of new species}
{\centering
...
Such a system $\mathcal{S}$, is composed of two equations,
 ...
 Where the \emph{kryptotype} $\kappa(t)$, is a list of a system's molecular concentrations at time $t$ -- or more generally the systems \emph{internal dynamics}. As some system dynamics may not be visible nor relevant to selection, we distinguish the \emph{kryptotype} (as it is hidden) from the \emph{phenotype} -- which is the systems \emph{external dynamics}.
  ...
  \end{align}
  where we sometimes refer to $h(t) := Ce^{A t}B$ as the \emph{impulse response} of the system. As $A$ specifies gene regulatory network architecture, the $i^\text{th}$ row of $A$ can be interpreted as the \emph{cis}-regulatory element (or promoter) for gene $i$. 
...
\end{tikzpicture}
    \caption{A graphical depiction of the set of all externally equivalent cell cycle control networks, $A(\tau)$. $\tau$ can take any value}
    \end{figure}
 % \end{example}
...
% \jss{added the phenotypic invariance stuff here (above) -- not sure if it should put elsewhere yet}
  ...
  It seems certain that selection is not so strong that intra-population variation
  ...
  It therefore seems reasonable to coarsely model the time evolution of population variation in network coefficients as 
