...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Therefore, since $e^{\Lambda s}$ is diagonal,
if we define $X_{ij}(t,Z)$ to be the $(i,j)$th entry of \eqref{eqn:exp_deriv}
multiplied by $U$ and $U^{-1}$,
\begin{equation}
  \begin{aligned}
    X_{ij}(t,Z) 
    &:=
      \left( \int_0^t e^{\Lambda s} U^{-1} Z U e^{\Lambda (t-s)} ds \right)_{ij} \\
    &=
      \int_0^t e^{\lambda_i s} \left( U^{-1} Z U \right)_{ij} e^{\lambda_j (t-s)} ds \\
    &= 
...
\begin{equation}
    \begin{aligned}
        D(A_0+\epsilon Z)
        &\approx \epsilon^2\sum_{ijk\ell} H_{ij,k\ell} Z_{ij} Z_{k\ell} 
    \end{aligned}
\end{equation}
and so
\begin{equation}
