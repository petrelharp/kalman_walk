...
\begin{scope}[>={Stealth[black]},
              every node/.style={fill=white,circle},
              every edge/.style={draw=black, thick}]
    \path [->, sloped] (A) edge[bend left] node {$a_{2,1}$} (B);
    \path [->, sloped] (B) edge[bend left] node {$a_{1,2}$} (A); 
   % \path[->] (U) edge[dashed] node {$b_{1}$} (A);
   % \path[->] (U) edge[dashed] node {$b_{2}$} (B);
   % \path[->] (A) edge[dashed, bend right] node {$c_{1}$} (y);
   % \path[->] (B) edge[dashed, bend left] node {$c_{2}$} (y);
\end{scope}
\begin{scope}[>={Stealth[black]},
              every edge/.style={draw=black, thick}]
    \path [->] (A) edge[loop left] node {$a_{1,1}$} (A);
    \path [->] (B) edge[loop right] node {$a_{2,2}$} (B);
\end{scope}
