...
% are these good here? {taylor2016diverse, matsui2015regulatory}.
...
but the population of $F_2$s will have variance of order $\sigma^2_S + \sigma^2_H T/N_e$,
where $\sigma^2_H$ is a factor that depends on the genetic basis of the coefficients.
\plr{say something about $\sigma_H$?}
...
The fitness consequences of system drift above were due to increased \emph{variance} in $F_2$ hybrids.
If the optimal set $\allS$ is curved (which it will be in general),
then the mean $F_1$ offspring will fall away from the optimum slightly,
leading to an additional fitness cost in hybrids.
However,
as shown in appendix \ref{apx:away_from_opt} and depicted in figure \ref{fig:osc_incompat},
the deviation of the mean from the optimum grows proportionally to $T/N_e$.
...
\end{align*}
With $\sigma = c_\sigma \gamma \sqrt{T/N_e}$
and $\mu = c_\mu \gamma (T/N_e)^2$,
this is
$$
...
%%%%%%%%%%%%
\plri{insert commented-out discussion of what the actual genetic basis is}
...
If $\widetilde \Sigma = (\Sigma^{-1} + U^{-1})^{-1}$ 
...
\begin{align*}
    \mu &\mapsto \mu' = (\Sigma^{-1}+U^{-1})^{-1} \Sigma^{-1} \mu \\
    \Sigma &\mapsto \Sigma' = \frac{1}{2} (\Sigma^{-1} + U^{-1})^{-1} + R .
\end{align*}
What measures are stable under this transformation?
The condition $\mu = \mu'$ reduces to $\Sigma U^{-1} \mu = 0$;
if we assume $R$ and therefore $\Sigma$ are of full rank,
then this happens if and only if $\mu$ is not in the span of the columns of $U$,
...
$\Sigma U^{-1} \Sigma + (I/2 - R U^{-1}) \Sigma = R$.
We can find a more explicit description
if we assume that $x^T U x = \sum_{i=1}^k x_i^2$,
...
computed from $R$ and $U$ as above,
...
Defining $\Gamma = (I - (I + \Sigma U^{-1})^{-1})$,
...
\end{align*}
% Note that after some algebra, $\Gamma = \Sigma U^{-1} (I - \Sigma U^{-1})^{-1}$.
...
\end{align*}
Since the second line doesn't depend on $x$,
by substituting $A=\Sigma^{-1}$ and $B=U^{-1}$,
\begin{align*}
    \frac{ f(x;\Sigma,y) f(x;U,0) }{\int f(z;\Sigma,y) f(z;U,0) dz}
    &=
...
\plr{Fix this calculation to include derivatives wrt to $B$ and $C$ also.}
