...
as the genetic drift of small mutational tweaks to these molecular pathways
cause a population to explore the set of mechanisms with identical phenotypes.
In this setting we treat an organism as a ``black box''
...
Within this framework, we show that there is never a unique network architecture for any phenotype
and that the evolutionary exploration of these distinct and mutationally connected mechanisms
can lead to the reproductive incompatibility of idependently evolving populations.
  This evolutionary exploration, which we refer to as system drift, proceeds at a rate proportional to the effective population size ($N_e$) and amount of intrapopulation genetic variation.
We estimate that %in reasonably numerous species,
  this process can lead to the formation of new species, perhaps in as few as $0.1 N_e$ generations.
%over a number of generations proportional to the effective population size.
This model also naturally yields, what appears to be, a distinct explanation for Haldane's rule, or why heterogametic hybrids tend to be disrupted more often than homogametes during the early stages of speciation.
\end{abstract}
...
It is an overarching goal of many biological subdisciplines to attain a general understanding of the function and evolution of this molecular machinery. 
For example, 
...
as well as thoughtful verbal and conceptual models \citep{true2001developmental, gwagner1, weiss2000phenogenetic, edelman2001degeneracy, pavlicev2012model}. 
...
even if a one-to-one mapping between model and behavior is possible in theory,
  even tiny amounts of noise can make inference problems nonidentifiable in practice \jss{cite?}.
...
\citep{kimura1981possibility, true2001developmental, tanay2005conservation, tsong2006evolution, hare2008sepsid, vierstra2014mouse, matsui2015regulatory, dalal2016transcriptional, dalal2017transcription}.
...
We study the evolution of an optimally adapted population subject to stabilizing selection for phenotype -- that is the population evolves under constant selective and environmental pressures.
Even if the phenotype remains stable over evolutionary time, the underlying mechanism might not remain so,
%Even when stabilizing selection preserves phenotype over evolutionary time, the underlying mechanism may change, 
...
\section*{Evolutionary system theory}

Here we construct as abstract model to study biological systems, such as gene regulatory networks. The behavior (i.e. temporal dynamics) of such a system is determined by a collection of $n$ coregulating moleculues -- such as transcription factors, as well as external or environmental inputs.
...
The $m$ ``inputs'' determined exogenously to the system are denoted $u(t)$,
and the $\ell$ ``outputs'' are denoted $\phi(t)$.
      ...
      \centering
       %  \begin{tabular}{cc}
            ...
            \begin{scope}[every node/.style={circle,thick,draw}]
                ...
                \node[shape=rectangle] (y) at (2,-2) {\begin{tabular}{cc} output ($\phi$) \\  \includegraphics[width=0.15\textwidth, height=0.04\paperheight]{osc_impulse} \end{tabular}};
            \end{scope}
...
Some gene networks, however, can grow or shrink, perhaps following gene duplications and deletions, 
...
The oscillator can also be equivalently realized by a three-gene (or larger) network, and will have even more evolutionary degrees of freedom available, as in e.g., Figure \ref{fig:3D_osc}.
...
\begin{scope}[>={Stealth[black]},
              every edge/.style={draw=black, thick}]
    \path [->] (A) edge[loop below] node[sloped, anchor=center, below] {\tiny $1$} (A);
    \path [->] (B) edge[loop above] node[sloped, anchor=center, above] {\tiny $1$} (B);
    \path [->, >=Rectangle] (C) edge[loop right] node[sloped, anchor=center, above] {\tiny $-1$} (C);
\end{scope}
\end{tikzpicture}
  \caption{A possible non-minimal three-gene oscillator phenotypically equivalent to the systems in Examples \ref{ex:oscillator} and \ref{ex:all_osc}.}
  \label{fig:3D_osc}
\end{figure}
...
despite both parents being well adapted. If this is consistent for the entire population, we would consider them to be separate species, in accord with the biological species concept \citep{mayr2000biological}.

Diploid organisms have two genomes, each of which encodes a set of system coefficients.
We assume that a diploid which has inherited systems $(A', B', C')$ and $(A'', B'', C'')$ from each of its parents,
...
With these definitions, an $F_1$ offspring
carries a system copy from each parent,
...
\begin{figure}[H]
  ...
  \caption{
    ...
    \textbf{(right)} Phenotypes of all 256 possible $F_2$ hybrids between the same set of parents,
...
\end{figure}
  ...
  \label{fig:osc_incompat}
   ...
   % \caption{
    ...
    for $F_1$ (red) and $F_2$ (blue) hybrids between an $A(0)$ and an $A(\epsilon)$ parent. Genetic distance is computed as the average coefficient difference between two systems.}
...
$\left( \left[ \begin{smallmatrix} A_1 & A_2 \\ \cdot & \cdot \end{smallmatrix} \right], \left[ \begin{smallmatrix} A_1 & A_2 \\ X_1 & X_2 \end{smallmatrix} \right] \right)$
  has phenotype determined by $A = \left[ \begin{smallmatrix} A_1 & A_2 \\ X_1 & X_2 \end{smallmatrix} \right]$, thanks to \emph{Drosophila}-like dosage compensation mechanism (the X is twice upregulated in heterogametes relative to homogametes),
...
$F_1$ males are effectively equivalent to purely autosomal-system $F_2$s. As such, in this setting, Haldane's rule is mechanistically similar to \emph{hybrid breakdown}, the observation that $F_2$s will often be less fit than $F_1$ hybrid crosses, in the early stages of speciation.
This mechanism appears to be distinct from the ``dominance'', ``faster-X'', and ``faster-male'' theories \citep{orr1997haldane, coyne1998evolutionary, turelli1995dominance} usually suggested to explain Haldane's rule.
...
We will suppose that each organism's phenotype is determined by its vector of coefficients, denoted by $x=(x_1, x_2, \ldots, x_L)$,
...
As above, an individual's coefficients are given by averaging its parentally inherited coefficients and adding random noise due to segregation. 
...
In this context, the Kalman decomposition \citep{kalman1963mathematical} implies that nearly all systems are nonidentifiable,
and gives an analytical description of all phenotypically equivalent gene networks.
System nonidentifiability, rather than being a computational nuisance,
implies the existence of axes of genetic variation that are not constrained by selection.
...
system drift is a significant -- and often rapid -- driver of speciation.
...
While the genetic incompatibilities contributing to hybrid inviability described in this paper share similarities with those in the commonly discussed Dobzhansky-Muller model, they may in fact share more in common with what has been described as the ``pathway model'' \citep{lindtke2015genetic}.

Consistent with empirical observation of \emph{hybrid breakdown} \citep[e.g.][]{plotner2017chlorosis}), we see that the fitnesses of $F_2$ hybrids drop at a much faster rate than those of $F_1$s,
...
System drift and network rewiring has been inferred across the tree of life \citep{dalal2017transcription, johnson2017rewiring},
and there is often significant regulatory variation segregating within populations \citep{XXX}.
\js{cite sergey?}
Transcription in hybrids between closely related species with conserved transcriptional patterns can also be divergent 
\citep{haerty2006gene, maheshwari2012cis, coolon2014tempo, michalak2004association}.
...
%
%The model we develop here is perhaps a plausible mechanism for strictly \emph{neutral} speciation.
%This is perhaps surprising, as speciation is often thought to simply be a byproduct of adaptative change \citep{schluter2009evidence, orr2004speciation}
%%as much of the speciation literature to date
%%has focused on positive selection and/or genetic conflict,
%or due to genetic conflict,
%and since previous authors have argued that neutral processes should be too slow
%\citep{nei1983models,seehausen2014genomics}.
%In most cases to date where the precise molecular mechanism underlying hybrid incompatibility is known,
%the genes responsible are thought to have been subject 
%to recent positive selection \citep{examples} 
%and/or genetic conflict \citep{examples}.
%\plr{get examples from \citet{orr2004speciation}}
%%However, incompatibility due to more diffuse system breakdown
%%(e.g., widespread misregulation \citep{coolon2014tempo} \plr{check that} \jss{I don't think this is the right citation})
%%may be likely much more difficult to uncover.
%% \citet{orr2004speciation} remarks that hybrid incompatibility data suggests that ``speciation results from positive Darwinian selection within species''. 
%On the other hand, the Dobzhansky--Muller model of hybrid incompatibility 
%is agnostic to the origin of the genetic substitutions \citep{coyne1998evolutionary} \plr{cite bateson, etc},
%and other plausible mechanisms for the origin of Dobzhanzky-Muller incompatibilities by neutral genetic drift
%have been proposed \citep[e.g.][]{lynch_force}.
...
and plausible mechanisms for the origin of Dobzhansky--Muller incompatibilities by neutral genetic drift have been proposed \citep{lynch_force} 
...
Using simulations, \citet{porter2002speciation} demonstrated the accumulation of hybrid incompatibilities under directional, but not stabilizing selection, and \citet{palmer2009dynamics} observed the appearence of incompatibilities in suboptimally adapted populations in a constant environment. 
...
or a conseuqence of genetic conflict \citep{presgraves2010molecular, crespi2013conflictual}.
In contrast, the model we develop here, suggests that even under strictly neutral conditions, system drift can lead to speciation, perhaps at a rate fast enough to play a role in species formation across the tree of life. 
%In contrast, the model we develop here, is perhaps a plausible mechanism for strictly \emph{neutral} speciation. 
%
%In practice it remains unclear whether system drift or other speciation mechanisms predominate, 
%as hybrid incompatibilities due to both selection and conflict have been uncovered and hint at general processes \citep{orr2004speciation}. 
%While it is known that divergent selection and genetic conflict, as well as other mechanisms, can precipitate speciation, we have shown here that, surprisingly, neutral drift may also significantly drive species formation.
Our results show that hybrids, under neutral processes, break down as a function of genetic distance, and despite the present model's myriad simplifications and assumptions, 
it may, in part, explain the observed broad patterns of reproductive isolation, such as the observed consistent relationship between molecular divergence and genetic isolation across diverse taxa and ecologies by \citet{roux2016shedding}, and the suggestivley nonadaptive, clocklike speciation patterns observed by \citet{hedges2015tree} (as diversification is not observed to be accelerated following mass extinctions).
While the incompatibility of any one given network within an organism may be small, we note that organisms are made up of many different networks, and that the cumulative impact of system drift across all of these may be substantial. Further, even in populations experiencing directional selection, networks underlying conserved parts of the phenotype can still experience neutral drift and harbor incompatibilities. 
...
implying saturating response curves \citep{michalis_mentin}.
...
suggests qualitatively similar results.
Furthermore, nonidentifiability (which implies the existence of neutral directions)
is often found in practice in moderately complex models of biological systems. \plr{need citation}

Finally, despite our model's precise separation of phenotype and kryptotype, this relationship in nature may be far more complicated as aspects of the kryptotype may be less ``hidden'' than we currently assume. For instance, attributes excluded from the phenotype as modelled here, ignore the potential energy costs associated with execessively large (nonminimal) kryptotypes, as well as the relationship between a specific network architecture and mutational robustness, as well as many other presently neglected phenomena. Despite the model's simplifications and potentially nontrivial oversights, it may still roughly reflect and shed light upon speciation patterns as they occur in nature.

%\paragraph{Peter: Quant gen discussion}
