...
\plr{note: should be a plot of the components of this against $T/N_e$ later}
...
the fitness of $F_2$s is $\fit_2 \le 1/\sqrt{1 + 4 \omega a^2 T/N_e} \approx 1 - 2 \omega a T/N_e$.
Since in small populations at mutation-drift equilibrium
the amount of genetic variance ($\sigma_N$) is proportional to $N_e$,
so this predicts the rate of decrease of hybrid fitness to be \emph{independent} of population size
for small populations.
\plr{look up what $\sigma_N^2$ is at mut-drift equilib}
...
Then the fitness of $F_2$s is $\fit_2 \le 1/\sqrt{1 + 2 \omega a^2 T/N_e} \approx 1 - \omega a T/N_e$,
and could be extremely rapid if $a$ is large.


but not likely to be other combinations; make plot of \eqref{eqn:fit2} against $T/N_e$ (or even $T$?) in these three situations.
...
with $\omega=1$ and 
\textbf{(A)} $\sigma_N = \sigma_S = \gamma$
\textbf{(B)} $\sigma_N = \sigma_S = 0.1 \gamma$
\textbf{(C)} $0.1 \sigma_N = \sigma_S = \gamma$.
