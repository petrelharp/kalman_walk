...
Above we synthesize models from molecular and evolutionary biology with tools from control theory
to study the evolution of a molecularly mechanistic model of the genotype-phenotype map under stabilizing selection.
This model allows us to describe analytically the processes described verbally as
...
System drift has been inferred across the tree of life \citep{johnson2017rewiring},
...
Although, as classically formulated, the Dobzhansky-Muller model of hybrid incompatibility is agnostic to the relative importance of neutral versus selective genetic substitutions \citep{coyne1998evolutionary} (as the neutral theory did not yet exist), \citet{orr2004speciation} remarks that hybrid incompatibility data suggests that ``speciation results from positive Darwinian selection within species'' thus maintaining one of the ``central tenets of the modern synthesis''. 
Reviewing both theoretical \citep{nei1983models} and empirical results on speciation, \citet{seehausen2014genomics} suggest that genetic drift is too slow to drive speciation and instead interpret genomic conflict to be a more significant driver.

\emph{Ecological speciation} as espoused by Schluter (and sometimes referred to as \emph{parallel speciation} \citep{schluter1995parallel}) suggests that reproductive incompatibility is a byproduct of adaptation -- and stands in stark contrast to the model presented here -- as it specifically predicts that two independent and initially identical populations evolving under stabilizing selection will \emph{not} harbor incompatibilities \citep{schluter2009evidence}. This is not to say that our model does not predict that ecological divergence can drive speciaiton -- if adaptation manifests as a significant difference in optimal (and thus evolved) phenotypes ($\phi \neq \bar \phi$) between initially identical populations, hybrids between the two will surely be less fit in both parental environments, however, this type of incompatibility will be function/network specific and not necessarily entail general steriliy or developmental inviability (so long as the particular networks underlying these processes did not adapt to novel ecologies) in the absense of system drift. \jss{should I write out an example including the oscillator? Such as, if natural selection shifts the frequencies between adapting populations, hybrids will exhibit neither optimal frequncy of oscillation, however under the ecological speciation model, we would not expect to find incompatibilities in any other of the organism's gene networks.}
...
While it is known that divergent selection and genetic conflict, as well as other mechanisms, can precipitate speciation, we have shown here that, surprisingly, neutral drfit may also significantly drive species formation.

If system drift induced hybrid incompatibility occurs as rapidly as our model sugests,
%even in the absence of postive Darwinian selection, 
this would imply that species originate not necessarily by means of natural selection \citep{darwin1859origin}. While the incompatibility of any one given network within an organism may be small, we note that organsisms are made up of many different networks, and that the cumulative impact of system drift across all of these may be substantial. Further, even in populations experiencing directional selection, networks underlying conserved parts of the phenotype can still experience neutral drift and harbor incompatibilities. 
