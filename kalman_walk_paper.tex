...
Even if a species' phenotype does not change over evolutionary time, the underlying mechanism may change, as distinct molecular pathways can realize identical phenotypes.
...
There is a substantial amount of work studying the evolution of gene regulatory networks, in both abstract \citep{wagner1994evolution, wagner1996does, siegal2002waddington, bergman2003evolutionary, draghi2015robustness} 
and empirically inspired computational
\citep{mjolsness1991connectionist, jaeger2004dynamic, vitaly1, crombach2016gap, wotton2015quantitative, chertkova2017insilico} frameworks.
...
We study the evolution of an optimally adapted population under stable conditions, that is where phenotype is under stabilizing selection.
...
system drift may result in reproductive incompatibility between sufficiently isolated populations,
...
reproductive incompatibility due to system drift manifests.
...
% barton1989divergence not available but likely following on from barton1986maintenance analyzing multiple optima in a similar model
...
% barton2001role looks at hybrids betw pops adapting to shifting optima in Fisher's geometric model 
% fraisse2016genetics looks at stabilizing selection using Fisher's geometric model, showing it predicts lots of observations
% chevin2010niche looks at incompats after adaptation in Fisher's geom model and how it depends on dimension
...
\citep{wright1935evolution,barton1986maintenance,barton2001role},
...
Despite the findings of these quantitative genetics models, previous simulations of system drift in regulatory sequences \citep{tulchinsky2014hybrid}
and a regulatory cascade \citep{porter2002speciation}
found that speciation could be rapid under directional selection
...
It is known that hybrid incompatibility can result from the increased segregation variance of a quantitative trait \citep{slatkin1994segregation, chevin2014niche}. Here, by focusing on the explicit biological function or physiology of a phenotype, we show that the evolution of even extremely simple phenotypes can appropriately be approximated by quantitative genetics. 
This is because, like the genetic code, the genotype-phenotype ``code'' is fundamentally degenerate -- as we make explicit -- and thus ammendable to quantitative genetic analysis. 
...
Using gene regulatory network dynamics as the basis for our models, we begin with a collection of $n$ coregulating molecules
...
instead, it derives from the nature of segregation load.
...
In this paper, we use tools from quantitative genetics and control theory
...
There has been substantial work on models of quadratic stabilizing selection
around a single optimum -- i.e., Fisher's geometric model \citep[e.g.][]{simon2017coadapted}.
Our model would have this form if viewed only in phenotype space, 
where there is a single optimum --
but differs by explicitly considering evolutionary dynamics of underlying genotype-derived regulatory matrices.
These make the quantitative genetic model we are led to
a generalization of Fisher's geometric model,
with distinct behavior,
due to the existence of neutral directions in genotype space.
However, there are many shared aspects:
for instance, \citet{barton1989divergence} also found incompatibilities accumulating
with genetic drift.
Fisher's geometric model also predicts Haldane's rule
and many other empirically observed patterns \citep{fraisse2016genetics},
...
\section*{Acknowledgements}
    ...
    We would also like to thank Nick Barton, Sarah Signor and Todd Parsons for very helpful comments on the manuscript.
