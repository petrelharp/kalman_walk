...
A key difference from the dominance theory is that this model treats system coefficients as quantitative traits. The classification of alleles as either ``dominant'' or ``recessive'' fits most naturally for Mendellian traits. Along these lines, if we think of individual loci of large effect then the effect of mutations on fitness (if convex) will usually be recessive, congruent with the dominance theory's predictions. However, the dominance or recessiveness of even large effect mutations will depend on both the magnitude of their effect and precisely how they adjust system coefficients. For such mutations that keep systems on the neutral manifold (imagine mutations to the oscillator that only move $\tau$), the dominance coefficient (usually notated as $h$) will be mathematically undefined. Mutations that driectly perturb coefficiens of $A$ can be either dominant or recessive, depending on their size.

\citet{barton2001role} explored Haldane's rule in the context of a quantitative trait and concluded that it was a consequence of the dominance theory. However, Barton also demonstrated $F_2$ hybrid breakdown in a model with strictly \emph{additive} inheritence. The model suggested here explains Haldane's rule as a special case of hybrid breakdown. As hybrid breakdown can occur for a quantitative trait under additivity, so can heterogametic inviability. This suggests that the model presented here is distinct from the dominance theory. 
