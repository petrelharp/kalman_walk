...
Distribution of system coefficients across the autosomes and sex chromosomes does not alone guarantee Haldane's rule.
Under this model, Haldane's rule will only occur when there is sufficient genetic distance seperating parentals
and when the geometry of the optimal set produces $F_2$ hybrid breakdown. \revpoint{2}{1}
...
Experimental approaches designed to study the effect of drift on speciation which induced population bottlenecks found no reproductive isolation in the majority tested pairs\citep{fry2009laboratory, white2020past}. These bottlenecks might not maintain enough genetic variation for systems to drift at an appreciable rate, however. \revpoint{1}{1}
%\jss{Did you also want to add something about bottlenecking large versus small popoulations?}
%and incompatibilities identified between \emph{Drosophila simulans} and \emph{D. melanogaster} exhibit evidence of adaptative evolution \citep{tang2009evolution}.
%
...
More recently, \citet{sanchez2021widespread} observed widespread transcriptional misregulation in \emph{Drosophila} hybrids, implicating the action of system drift.

Selection could dramatically accelerate speciation by essentially the same process --
...
Fluctuating selection \citep{bell2010fluctuating, barton2001role} is ongoing, so could further accelerate speciation.
Under our model, populations evolving towards different optimal phenotypes will always produce less fit hybrids,
as the hybrid phenotypes will be displaced from both parental optimums -- that is maladapted to both parental environments. This suggests that fluctuating selection -- if not synchronized between populations -- should diminish hybrid fitnesses.
In the case where fluctuations are synchronized, due to the size of the neutral system space, 
populations may arive at different system organizations when returning to previous phenotypes at a rate faster than by drift alone. \revpoint{1}{3}
...
possibly due to epistasis, genetic architecture \citep{hermisson2003epistasis}, adaptive inertia \citep{baatz1997adaptive,alvarez2009effects} or weak gene flow could plausibly slow it down. \revpoint{AE}{1}\revpoint{AE}{3}
...
Our results describe a continuum of possible systems,
described by their continuous coefficient matrices.
...
\citep[as in, e.g.,][]{martin2014fishers,fraisse2016genetics,simon2017coadapted}
might provide more quantitative, generalizable predictions.
It is also natural to ask about the distribution of effect sizes of individual mutations
in a population evolving under this model,
and what the distribituion of selection coefficients would be
(at equilibrium, perhaps).
These are questions that might be best answered by simulation,
and could provide much additional information -- for instance,
are alleles typically dominant in their effects on system coefficients or fitness?
