...
\section{Linear Evolution-Variant Systems}
  Let $V(\tau) = \prod_{\ell = 0}^{\tau} (I + Q_\ell)$ (where $Q_\ell$ is a random matrix with norm $< \sigma$), and let $A$ be minimal. Then the system architecture after evolutionary time $\tau$ is given by,
    \begin{align}
      \mathcal{S}(\tau) := \left\{ \begin{array}{ll} V(\tau)\dot{\kappa}(t) &= V(\tau)A V(\tau)^{-1} \kappa(t) + V(\tau)B u(t) \\ \phi(t) &= C V(\tau)^{-1} \kappa(t) \end{array} \right .
  \end{align}
