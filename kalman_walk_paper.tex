...
In this paper we use results on mathematical nonidentifiability from \hypertarget{rev2:1}
...
\begin{example}[An oscillator]\label{ex:oscillator}\hypertarget{rev1:1}
...
The Kalman decomposition then classifies each direction in kryptotype space\hypertarget{rev2:3}
...
both the change of basis used to obtain the decomposition\hypertarget{rev2:4}
...
\paragraph{Haldane's rule}\hypertarget{rev1:5}
...
which are less fit on average than $F_1$s (see Figure \ref{fig:osc_incompat}).
\revpoint{1}{4}
...
Consistent with empirical observation of hybrid breakdown %(e.g., \citet{plotner2017chlorosis}),\hypertarget{rev1:6} 
...
Phenotypically equivalent system evolution is probably not only driven by neutral genetic drift. \hypertarget{rev1:4}
