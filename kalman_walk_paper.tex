...
For any input $u(t)$,
suppose that $(\kappa(t),\phi(t))$ and $(\bar \kappa(t),\bar \phi(t))$ are the solutions to equation \eqref{eqn:system}
...
\end{align*}
This occurs if and only if their impulse response functions are the same:
if $h(t) = \bar h(t)$ for all $t \ge 0$.
...
If the system is not minimal, then the Kalman decomposition tells us XXX
