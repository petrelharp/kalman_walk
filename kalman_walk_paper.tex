...
Phenotypically equivalent gene networks are not necessarily compatible with one another, thus system drift can rapidly lead to the accumulation of hybrid incompatibilities analogous to those in the Dobzhansky-Muller model. 
...
Here we construct as abstract model to study biological systems, such as gene regulatory networks. The behavior (i.e. temporal dynamics) of such a system is determined both by a collection of $n$ coregulating moleculues -- such as transcription factors, as well as external or environmental inputs.
      ...
      \centering
         \begin{tabular}{cc}
            ...
            \begin{scope}[every node/.style={circle,thick,draw}]
                ...
                \node[shape=rectangle] (U) at (2,2) {input ($u(t)$)};
                \node[shape=rectangle] (y) at (2,-2) {output ($\phi(t)$)};
            \end{scope}
...
\begin{scope}[>={Stealth[black]},
              every node/.style={fill=white,circle},
              every edge/.style={draw=black, thick}]
    \path [->, sloped] (A) edge[bend left] node {\tiny ?} (B);
    \path [->, sloped] (B) edge[bend left] node {\tiny ?} (A); 
    \path [->, sloped] (A) edge[bend right] node {\tiny ?} (C); 
    \path [->, sloped] (B) edge[bend left] node {\tiny ?} (C); 
    \path [->, sloped] (C) edge[bend right] node {\tiny ?} (A); 
    \path [->, sloped] (C) edge[bend left] node {\tiny ?} (B); 
    \path[->] (U) edge node {\tiny $1$} (A);
    \path[->] (U) edge node {\tiny $1$} (B);
...
\begin{scope}[>={Stealth[black]},
              every edge/.style={draw=black, thick}]
    \path [->] (A) edge[loop below] node[sloped, anchor=center, below] {\tiny ?} (A);
    \path [->] (B) edge[loop above] node[sloped, anchor=center, above] {\tiny ?} (B);
    \path [->] (C) edge[loop right] node[sloped, anchor=center, above] {\tiny ?} (C);
\end{scope}
\end{tikzpicture}
...
\jss{Maybe something like dosage compensation drift could also lead to Haldane's rule. That is, if the dosage compensation can either be: upregulate the X by $2$ in male flies, versus in another species: downregulate the X in females by $1/2$, and all possible combinations inbetween?}
