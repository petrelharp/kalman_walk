...
In this paper we use results on mathematical nonidentifiability from \llabel{r21}
...
in a similar vein as \emph{holey landscape} models \citep{gavrilets1997evolution, yamaguchi2013first}.

% It is not a new observation
The field of population genetics has also explored the consequences of the fact
%that phenotype does not prescribe a unique genotype
that there is often more than one way to do the same thing, \revpoint{1}{2}
\llabel{r22}
...
that can make natural predictions of these model parameters
...
\begin{example}[An oscillator]\label{ex:oscillator} \llabel{r11b}
...
Results in system theory show that any time-invariant \revpoint{2}{5}
system can be realized in a particular minimal dimension
...
The Kalman decomposition then classifies each direction in kryptotype space\llabel{r23}
...
both the change of basis used to obtain the decomposition\llabel{r24}
...
Although the other submatrices do not affect the phenotype
through the impulse response function $h(t)$,
they may affect individual fitness in other ways \revpoint{2}{4}
(which we don't consider here).
For instance,
the ``unreachable'' subspaces cannot be affected by any of the possible variations
in input produced by varying $u(t)$.
However, if the nature of the input changed
(for instance, by perturbing a previously constant environmental variable),
differences in the unreachable components might lead to differences in phenotypes
and thus represent cryptic genetic variation.
Similarly, ``unobservable'' subspaces might represent aspects of phenotype
that are not constrained by selection,
but might become important if conditions change.
Additionally, all subspaces may affect how the system deals with noise.
...
Example \ref{ex:all_osc} gives the set of minimal systems equivalent to the oscillator of Example \ref{ex:oscillator},
...
\paragraph{Sexual reproduction and recombination}\llabel{r26}
...
$((A'+A'')/2, (B'+B'')/2, (C'+C'')/2)$.
(We do not, however, specify a model for recombination to produce the inherited systems,
as the details will depend on the genetic basis of the system coefficients;
two extremes are depicted in the two panels of Figure~\ref{fig:conceptual_fig}.) \revpoint{2}{6}

\jss{Could add something like this?}

In Example \ref{ex:hybrid_osc} and Figure \ref{fig:hybs}, however, meiosis is modeled such that it recombines system coefficients independently,
to form haploid gametes. E.g., a possible oscillator gamete could be formed by shuffling the diploid's haploid genomes -- with one genome's coefficients notated with Greek and the other with Latin, as follows,
$\left[ \begin{smallmatrix} \alpha & \beta \\ \gamma & \delta \end{smallmatrix} \right] \times \left[ \begin{smallmatrix} a & b \\ c & d \end{smallmatrix} \right] \to \left[ \begin{smallmatrix} \alpha & b \\ c & \delta \end{smallmatrix} \right]$.
...
\begin{figure}[H]
  \includegraphics{examples/blue_hybrids2}
  \caption{
    ...
    Parental phenotypes ($\sin t + \cos t$) are shown in solid black, and hybrid phenotypes in blue.
    ...
    $F_2$ hybrids, many of which show complete breakdown, are shown in shades of blue and grey. 
  } \label{fig:hybs}
\end{figure}
...
\paragraph{Haldane's rule}\llabel{r15b}
...
More generally, the contribution of sex chromosomes to the system coefficients of $F_1$ males
are effectively equivalent to those of purely autosomal $F_2$ hybrids,
which are less fit on average than $F_1$s (see Figure \ref{fig:osc_incompat}). \llabel{r15e}
\revpoint{1}{4} \revpoint{2}{1}
Note, however, that presence of genetic variation on sex chromosomes
does not alone guarantee Haldane's rule:
under this model, Haldane's rule will only occur when there is sufficient genetic distance separating parentals
and when the nature of genetic variation produces $F_2$ hybrid breakdown.
...
Consistent with empirical observation of hybrid breakdown \llabel{r16}
...
System drift and network rewiring has been inferred across the tree of life \citep{wotton2015quantitative, crombach2016gap, dalal2017transcription, johnson2017rewiring, ali2019novel},
...
the Dobzhansky-Muller model of hybrid incompatibility is agnostic to the relative importance of neutral versus selected genetic substitutions \citep{coyne1998evolutionary},
...
Supporting this view, experimental approaches designed to study the effect of drift on speciation
through induced population bottlenecks
found no reproductive isolation in the majority of tested pairs \citep{fry2009laboratory, white2020past}. \revpoint{1}{1}
%and incompatibilities identified between \emph{Drosophila simulans} and \emph{D.~melanogaster} exhibit evidence of adaptive evolution \citep{tang2009evolution}.
...
More recently, \citet{sanchez2021widespread} observed widespread transcriptional misregulation in \emph{Drosophila} hybrids,
which might be due to the action of system drift.

Although the phrase ``system drift'' as we use it implies neutral changes,
it does not mean that these changes are induced purely by neutral genetic drift.
Selection could dramatically accelerate speciation by the same process --
e.g., by genetic draft \citep{gillespie2000genetic} or pleiotropic selection.
For instance, \citet{mani1990mutational} showed that populations
developed incompatibilities as they independently evolved towards the same quantitative trait optimum,
as different alleles fix in different populations. \llabel{citemani}
Fluctuating selection has similarly been shown to accelerate speciation
in some models \citep{bell2010fluctuating,barton2001role}.
Under our model, populations evolving towards different optimal phenotypes will always produce less fit hybrids,
as the hybrid phenotypes will be displaced from both parental optima,
and so maladapted to both parental environments.
This suggests that fluctuating selection -- if not synchronized between populations -- should diminish hybrid fitnesses.
In the case where fluctuations are synchronized, due to the size of the neutral system space, 
populations may arrive at different system organizations when returning to previous phenotypes at a rate faster than by drift alone. \revpoint{1}{3}
...
the cumulative impact of system drift across the many different networks an organism relies on may be substantial. \llabel{rp11}
...
Phenotypically equivalent system evolution is probably not only driven by neutral genetic drift. \llabel{r14}
...
possibly due to epistasis, genetic architecture \citep{hermisson2003epistasis}, adaptive inertia \citep{baatz1997adaptive,alvarez2009effects}
or weak gene flow could plausibly slow it down. \revpoint{AE}{1}\revpoint{AE}{3}
...
\paragraph{Geometric models} \llabel{quantnote}
The model we work with describes a continuum of possible systems,
parameterized by continuous coefficient matrices.
The arguments we make (e.g., Figure~\ref{fig:conceptual_fig}) are often geometric in nature,
and so it would be interesting to ask whether Fisher's geometric model
might provide more quantitative, generalizable predictions,
as in for instance \citet{fraisse2016genetics,simon2017coadapted}.
\citet{martin2014fishers} argued that such an approximation would be often fruitful.
In the other direction,
it is also natural to ask about the distribution of effect sizes of segregating mutations
in a population evolving under this model with a realistic \emph{de novo} effect size distribution,
as well as what the distribution of fitness effects would be (both within and between populations).
These are questions that might be best answered by simulation,
and could provide much additional information -- for instance,
are alleles typically dominant in their effects on either system coefficients or fitness?
