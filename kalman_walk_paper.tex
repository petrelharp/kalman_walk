...
\end{equation}
  \begin{align}
    \mathcal{K} := \bigcup_{n=\min}^{\infty} \calA_n(\Sys_0)
  \end{align}
...
%
 \begin{example}[All Phenotypically Equivalent Cell Cycle Control Networks] \label{ex:all_osc}
    ...
    %\end{align*}
    \begin{align*}
      A(\tau) &= \frac{1}{\tau-1} \begin{bmatrix} \tau & -1 \\ 2 \tau(\tau - 1) + 1 &  -\tau \end{bmatrix} \ \forall \ \tau \neq 1
    \end{align*}
    \begin{figure}[H]
    \centering
% Nice tikz!!!
...
For instance, diploid system coefficients are the average of two haploid genomes $\Sys$ and $\Sys'$: $\Sys_{\text{diploid}} = \left(\Sys + \Sys'\right)/2$. Additionally, during meiosis, a diploid recombines its two genomes to form a new one: $\left(\Sys \times \Sys' \right) \rightarrow \Sys''$; by randomly swapping matrix rows between two systems $\mathcal{S}$ and $\mathcal{S}'$.

Even if two genomes $\Sys$ and $\Sys'$ are both in $\mathcal{K}$, their diploid and recombinant systems may not be in $\mathcal{K}$. This is because $\mathcal{K}$ is not required to be closed under averaging or recombination.
...
For example if the two phenotypically equivalent systems from Example \ref{ex:all_osc} hybridize, the offspring's phenotype will fail to oscillate completely. 
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth, height=0.125\paperheight]{expF1}
    \caption{$F_1$ hybrid (orange) and parental (blue) phenotypic oscillator dynamics for an $A(0)$ by $A(2)$ cross. The hybrid fails to oscillate and exhibits qualitatively different dynamics ($\phi(t)_{F_0}\! = \! \sin(t) \! + \! \cos(t)$, however $\phi(t)_{F_1} \! = \! e^t$).
    }
  \end{figure}
%
