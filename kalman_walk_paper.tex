...
%that phenotype does not prescribe a unique genotype\revpoint{2}{3}
...
Results in system theory show that any time-invariant\revpoint{2}{5} system can be realized in a particular minimal dimension
...
This model differs from the \emph{dominance theory} \citep{turelli1995dominance} in that it treats system coefficients as quantitative traits.
%The classification of alleles as either ``dominant'' or ``recessive'' fits most naturally for Mendellian traits. 
If we think of individual loci of large effect then the effect of mutations on fitness (if convex) will usually be recessive, congruent with the dominance theory. However, the dominance or recessiveness of even large effect mutations will depend on both the magnitude of their effect and precisely how they adjust system coefficients. 
%For such mutations that keep systems on the neutral manifold (imagine mutations to the oscillator that only move $\tau$), the dominance coefficient (usually notated as $h$) will be mathematically undefined. Mutations that driectly perturb coefficiens of $A$ can be either dominant or recessive, depending on their size.

\citet{barton2001role} explored Haldane's rule in the context of a quantitative trait and concluded that it was a consequence of the dominance theory. However, Barton also demonstrated $F_2$ hybrid breakdown in a model with strictly \emph{additive} inheritence -- a consequence of segregation variance. The model suggested here explains Haldane's rule as a special case of hybrid breakdown. As hybrid breakdown can occur for a quantitative trait under additivity, so can heterogametic inviability. This suggests that the model presented here is distinct from the dominance theory; like hybrid breakdown, Haldane's rule derives from the nature of segregation variance.
...
This has led some to conclude that hybrid incompatibility is typically a byproduct of positive selection \citep{orr2004speciation, schluter2009evidence}\hypertarget{rev1:2} 
...
Experimental approaches designed to study the effect of drift on speciation which induced population bottlenecks found no reproductive isolation in the majority tested pairs\citep{fry2009laboratory, white2020past}. These bottlenecks might not maintain enough genetic variation for systems to drift at an appreciable rate, however.\revpoint{1}{1} \jss{Did you also want to add something about bottlenecking large versus small popoulations?}
%and incompatibilities identified between \emph{Drosophila simulans} and \emph{D. melanogaster} exhibit evidence of adaptative evolution \citep{tang2009evolution}.\revpoint{1}{1}
%
...
Fluctuating selection \citep{bell2010fluctuating, barton2001role} could further accelerate speciation via system drift. 
...
the cumulative impact of system drift across the many different networks an organism relies on may be substantial.\hypertarget{rev1:3} 
