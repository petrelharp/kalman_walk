...
and empirically inspired
  \citep{mjolsness1991connectionist, jaeger2004dynamic, vitaly1, crombach2016gap, wotton2015quantitative, chertkova2017insilico}. \jss{Something, something Wagner model here?} 

 % In biological systems, the relationship between structure and function is often not invertible -- that is information about function is not usually sufficient to determine structure.
 % Non-invertiblility thus entails a diversity of functionally equivalent structures.
  Biological systems, at nearly all levels of organization, display considerable degeneracy, or (perhaps more descriptively) a capacity for isofunctional \emph{structural plasticity} -- 
  that is systems can be structurally different yet remain functionally equivalent \citep{edelman2001degeneracy}. 
  This is a consequence of the often non-invertibility (i.e. degeneracy) of the mapping between a system's structure and function.
...
all the way up to the convergent evolution of adaptive traits. Between these levels, structural plasticity can be found in the \emph{neutral networks}
  of nucleic acid sequences that map to an RNA secondary structure \citep{gruner1996analysis}\revpoint{1}{5} and amino acid sequences that fold similarly \citep{babajide1997neutral},
as well as in the thermodynamic stability of proteins \citep{hart2014thermodynamic}. Further examples include the vast space of functionally equivalent
...
Likewise, it is well known that in many contexts mathematical models
...
In this paper we use linear systems theory to model gene regulatory networks, linking structural plasticity 
with mathematical nonidentifiability. Although our focus is on linear systems, nonidentifability is by no 
means unique to such frameworks, and is a general property of many nonlinear models \citep{nonlinear}.

Structural plasticity has been argued to be necessary for natural selection \citep{edelman2001degeneracy}, 
as it may function as a mechanism for biological robustness and evolvability, or manifest as 
\emph{canalization} \citep{whitacre2010degeneracy}. Structural plasticity of the genetic code, and of genetic networks, 
for instance, can make sequences more fault-tolerant to mutations \citep{sonneborn1965degeneracy} and open up pathways to adaptation \citep{wagner2008robustness}.\jss{Is it clear what ``structural plasticity of the genetic code'', as opposed to ``degeneracy of the genetic code'', means here?}
Structural plasticity can also enable the evolution of neutral and nearly neutral molecular changes \citep{kimura1968genetic},
and perhaps even contribute to the formation of new species \citep{gavrilets2014models}. 

We are interested in the nature and evolutionary implications of structural plasticity and nonidentifiability in the context of gene regulatory networks.
...
If systems architectures are not functionally unique, can this open up neutral evolutionary paths, and do
  these paths manifest as \emph{developmental system drift} \citep{true2001developmental} and contribute to speciation?
...
Finally, we briefly outline and apply a quantitative genetics framework to estimate the speed at which system drift can lead to speciation via reproductive incompatibility.
In this approach, a population diffuses along the neutral ridges of a high-dimensional space of possible system parameters, in a similar vein as \emph{holey landscape} models \citep{gavrilets1997evolution, yamaguchi2013first, pina2019does}\revpoint{2}{12}. Our model is developed in more detail in a companion paper \citep{schiffman2020genetic}.
...
%We then briefly outline a quantitative genetics framework,
%which we further develop in a companion paper,
%which estimates how quickly system drift (and more generally, the presence of a fitness ridge)
%leads to reproductive incompatibility \citep{schiffman2020genetic}.
%
...
Under quite general assumptions (\emph{e.g.} the system is linear, time-invariant and in the zero-state), \revpoint{AE}{3}
...
with genetic variance $V_G$ in a population with effective size $N_e$
will move in $t$ generations a random amount whose variance is $t V_G / N_e$ \citep{quant_gen}.
...
Since we assume they are at a local figness optimum, without much loss of generality we can assume
...
To move the the system (the $A$ matrix) a given distance
...
is unlikely to produce a a set of coordinated changes;
...
\jss{I didn't think that our results depend on the kryptotype being in a higher dimensional space than the phenotype.
Any coordinate change on (A,B,C) will rewire the system. So as long as none of the system matrices are constrained, 
any minimal system should have an infinite number of realizations. Unless my system theory is rusty?}
...
and different traits often share a genetic basis.
...
For instance, there exists minimal realizations (\emph{e.g.} the oscillator in Example 1) where the dimension of the kryptotpye is larger
...
Of course we see many more degrees of freedom in nonminimal networks, but it is not yet clear if evolution commonly arrives at such structures.
There are some hints that this may be occuring: compared to the molecular circuitry of the cyanobacteria circadian rhythm,
the human clock appears to be highly and unneccessarily complex \citep{sancar2008intelligent}. 
The true ``dimensionality of trait space'' has been debated at some length in the literature
\citep{trait_space}.
Whatever it is, however, it seems likely to be highly degenerate,
