...
-- minimal, informally meaning that the system's phenotype is achieved with the fewest possible genes $(n=\min)$
...
Therefore varying $V$ can tweak the relationships between genes within a regulatory network, yet preserve the phenotype. Some gene networks, however, can grown or shrink, perhaps following gene duplications and deletions, and also still preserve their phenotypes. These changes cannot be described by $S(V)$, but are ecanpuslated by \ref{eqn:equivalence}, below.

More generally,
we denote by $\calA_n(\mathcal{S}_0)$ the set of all $n$-dimensional systems equivalent to $\mathcal{S}_0$:
...
\end{equation}
  \begin{align}
    \mathcal{K} := \bigcup_{n=\min}^{\infty} \calA_n(\calA_0)
  \end{align}
...
Two systems, even if in different dimensions, will have identical phenotypes if they are in $\mathcal{K}$. Further, there is no unique triple $(A,B,C)$ for the mapping $u \mapsto \phi$. This implies that the gene regulatory network architecture per phenotype/environment pair is never unique -- that is \emph{all} gene regulatory networks can drift/rewire. This set can be precisley defined and completely parameterized using the \emph{Kalman decomposition} \ref{apx:kalman}.

A large, adapted population will move around in $\mathcal{K}$ given environmental and selective stasis. Even if there is a conservation of phenotype through evolutionary time, the molecular mechanism underpinning the phenotype is not constrained and likely will not be conserved.
...
For instance, the phenotypic dynamics of a diploid organism are a function of its two haploid genomes $x$ and $y$: $P(x,y) = \left(x + y\right)/2$. Additionally, during meiosis, a diploid recombines its two genomes to form a new one: $\left(x \times y \right) \rightarrow z$; by randomly swapping matrix rows between two systems $\mathcal{S}$ and $\mathcal{S}'$.

Even if two genomes $x$ and $y$ are both in $\mathcal{K}$, their diploid systems $P(x,y)$, and their recombinant systems $P(z,z)$, may not be in $\mathcal{K}$. This is because $\mathcal{K}$ is not mathematically required to be closed under averaging and recombination.

If sexual recombination among systems drawn from $\mathcal{K}$ yields systems with divergent phenotypes, populations containing significant diversity in $\mathcal{K}$ will carry a large genetic load, and isolated populations may fail to produce hybrids with viable phenotypes. 
...
and a set $\optx$ of ``optimal'' coefficients that produce this phenotype ($\calA_n$ above).
...
measured so that the fitness of an organism with coefficents $x$ is $\fitx(x) = \exp(-\dph(\Phi(x),\optph)^2)$. \js{need to add $\gamma^{2}$ here?}
...
the population mean coefficient will move a random distance of size $\sigma/\sqrt{N_e}$ per generation, \js{since it moves at rate $\sigma \sqrt{T/N_e}$ isn't saying  $\sigma/\sqrt{N_e}$ \emph{per generation} not that clear?}
...
and variance equal to $\sigma$. \js{I think you mean $\sigma^{2}$?}
...
as before,
but variance equal to $\sigma^2 + z^2/2$, where $z$ is the distance between the parental populations. \js{Can you clarify why $z^{2}/2$? Is this because it's averaging two distributions?}
...
$\fit_2/\fit_0 = \exp(- T/(N_e\gamma^2))$.
  ...
  \jss{ Assume that selection acts directly against organisms whose regulatory coefficients are not on the optimum manifold. Further assume that selection constrains this diversity to be, on average, a distance of $m$ from optimum. We would expect $\sigma_S = m$. Furtheremore, two individuals within a population may both be on the optimum manifold, but at different points, on average of $\sigma_N$. Since individuals a distance of $\sigma_N$ apart will produce $F_2$s off of the manifold of $\mu = \frac{\sigma_N}{2}\sin2\theta$, recombination load may constrain $\sigma_N$ such that $\mu \leq m = \sigma_S$ (where $\theta$ is the slope of the manifold -- as $\theta$ approaches $0$ or $1$, $\sigma_N \rightarrow \infty$.). We can estimate $\sigma_N = \frac{2\sigma_S}{\sin2\theta}$.  
  }
%%%%%%%%%%%%%%%%%%%%%%%%
...
then $\beta > \sigma$.  
\plr{do we call this $\beta$ or $u$?} \js{$\beta$, $u$ is input.}
However,
...
\paragraph{The rate of speciation under neutrality in allopatric populations}
...
If the strength of selection is only a few multiples $\xi_1$ of the standing phenotypic diversity within a population $\sigma_S$, such that $\beta = \xi_1 \sigma_S$, and recombination load constrains the standing phenotype-neutral diversity $\sigma_N$ to be only a few multiples $\xi_2$ of $\sigma_S$, then $F_2$ fitness drop will be,
  ...
  This suggests that if a population contains as much phenotypic diversity as allowed by selection and if $\sigma_N$ is on the order of $\sigma_S$, reproductively isolated populations can speciate rapidly -- possibly on timescales shorter than $N_{e}$ generations.
