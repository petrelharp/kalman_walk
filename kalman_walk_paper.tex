...
More generally,
we denote by $\calA_n(\Sys_0)$ the set of all $n$-dimensional systems equivalent to $\Sys_0$:
...
Further, we refer to the set of all linear systems with impulse response $h(t)$, regardless of dimension, to be $\allS$,
  ...
  \end{align}
  \js{what do you like better for $\allS$? $\mathcal{A}$, $\mathcal{A}_{\infty}$, or $\mathcal{K}_{h(t)}$, or something else -- maybe a modification of $\Sys$? Using a script of ``A'' was more intuitive when we were leavng $B$ and $C$ invariant.}
Two systems, even if in different dimensions, will have identical phenotypes if they are in $\allS$. Further, there is no unique triple $(A,B,C)$ for the mapping $h: u \mapsto \phi$. This implies that the gene regulatory network architecture per phenotype/environment pair is never unique -- that is under these assumptions, \emph{all} gene regulatory networks can drift/rewire, and still conserve the phenotype. This set can be precisley defined and completely parameterized using the \emph{Kalman decomposition} (Appendix \ref{apx:kalman}).
...
\js{should the sentence of evolution of nearly minimal be in the paper? Or should we save this for our next paper?}

A large, adapted population will move around in $\allS$ under environmental and selective stasis. In the minimal dimension neutral evolution proceeds as a stochastic coordinate transformation on the elements in $\Sys_{\min}$. Even if there is a conservation of phenotype through evolutionary time, the molecular mechanism underpinning the phenotype is not constrained and likely will not be conserved.
...
and that
depending on $u_{2}$ and $u_{1}$ the organism will synthesize an appropriate $\phi_{1}$ and $\phi_{2}$. 
...
%\end{align*}
  $\Sys_{\min}(U) = (U \left[ \begin{smallmatrix} 0 & -1 \\ 0 & 0 \end{smallmatrix} \right] U^{-1}, U \left[\begin{smallmatrix} 1 & 0 \\ 0 & 1 \end{smallmatrix}\right], \left[\begin{smallmatrix} 1 & 0 \\ 0 & 1 \end{smallmatrix}\right]U^{-1})$. 
%
...
%$h(t) = \left[\begin{smallmatrix} 1 & - t \\ 0 & 1 \end{smallmatrix}\right]$.
  ...
  $\Sys(\small X_{1-3},V) \! =  \! \left( \! \begin{array}{ll}
     V \! \left[\begin{smallmatrix} X_{1} & X_{2} \\ 0 & A \end{smallmatrix}\right] \! V^{-1}, V \! \left[\begin{smallmatrix} X_{3} \\ B \end{smallmatrix}\right], \left[\begin{smallmatrix} 0 & C \end{smallmatrix}\right] \! V^{-1} \end{array} \! \right)
...
%    \end{align*}
  ($V$ can be any $4$-dimensional and invertible matrix, $X_{1-3} \in \R^{2 \time 2}$, and $(A,B,C) \in \Sys_{\min}$).
...
Phenotypically identical yet kryptotypically variant gene networks, when brought together and/or recombined during sexual reproduction, can lead to phenotypic inviability. 
    ...
    For instance, the diploid $\{\Sys, \Sys' \}$ has system architure $\Sys_{\text{diploid}} \! = \! \left(\Sys \! + \! \Sys'\right)\!/2$, the average of its two haploid systems. During meiosis a diploid recombines its two genomes to form a new one (a gamete): $\left(\Sys \! \times \! \Sys' \right) \rightarrow \Sys''$; by randomly swapping matrix rows between two systems (Appendix \ref{apx:recombination}). Then two gametes join to form an offspring (\emph{e.g.} $\{\Sys'',\Sys\} $).
...
%For example, if the two phenotypically equivalent systems $A(0)$ and $A(2)$ from Example \ref{ex:all_osc} hybridize, their offspring's phenotype will fail to oscillate completely. 
  \begin{figure}[H] 
    ...
    \caption{\textbf{Hybrid phenotypic breakdown} $F_1$ hybrid (orange) and parental (blue) phenotypic oscillator dynamics for an $A(0)$ by $A(2)$ cross from Example \ref{ex:all_osc}. The $F_1$ hybrid
%$\left(A_{F_1}\!\!=\!\!\tfrac{A\!(\!0\!) \! + \! A\!(\!2\!)}{2}\right)$
gene expression fails to oscillate and instead grows exponentialy,
exhibiting qualitatively different dynamics ($\phi_{F_0}\! = \! \sin t \! + \! \cos t$, however $\phi_{F_1} \! = \! \exp t$).
    }
    \label{fig:expF1}
  \end{figure}
%
...
\js{commented out questions here b/c we don't answer them directly -- what do you think?}
...
\js{since it moves at rate $\sigma \sqrt{T/N_e}$ isn't saying  $\sigma/\sqrt{N_e}$ \emph{per generation} not that clear?}
...
\js{Can you clarify why $z^{2}/2$? Is this because it's averaging two distributions?}
...
%  where $\xi := 2\xi_2/\xi_1$.
  This suggests that, as an upper bound, if a population contains as much phenotypic diversity as allowed by selection, reproductively isolated populations can speciate rapidly -- possibly on timescales shorter than $N_{e}$ generations.
%%%%%%%%%%%%%%%%%%%%%%%%
