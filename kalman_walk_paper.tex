...
\plr{Saying "stalled" reflects negatively on other fields - rephrase?}
...
\plr{I don't think the HW example will speak to our audience.  Also, better to say why math is good rather than why not-math is bad.}
However, as Hardy and Weinberg taught us over a century ago, verbal theories are often insufficient, if not downright misleading \citep{hardy1908mendelian, weinberg1908vererbungsgesetze, servedio2014not}. 
...
\plr{not clear what you mean by "actually"}
...
\plr{rather than "we expect" maybe say that the framework could be applied to non-neutral evolution}
We expect wide applicability to this approach (\emph{i.e.} non-neutral evolution), 
however presently, we focus solely on neutral evolution, 
\plr{here's the definition of neutral, but it's not quite right?}
that is where phenotype is conserved over evolutionary time. 
  
\plr{We don't "derive" it, we point out that Kalman already derived it.}
We derive an analytical description of the set of all linear biological systems with identical phenotypes 
\plr{We need something here saying what we mean by "gene network architecture".  Also, I don't think we claim anything for "all biological systems".}
-- that is we describe the set of all gene network architectures that yield identical phenotypes, and show that all biological systems can, in principal, can undergo systems drift. 
...
%}
Here we outline a method to model biological systems, such as gene regulatory networks, as linear dynamical systems. 
We define an organism's ``phenotype'' to be the time dynamics of molecular concentrations directly relevant to survival, 
\plr{tried to say it more precisely. better phrase than "component of phenotype"?}
and denote by $\phi_k(t)$ the concentration of the $k^\text{th}$ component of phenotype at time $t$.
...
and an environmental input $u(t)$.
\plr{say what $u$ is (a vector)}
\plr{say in words what $\kappa$ is before the equation.}
Such a system $\mathcal{S}$, is composed of two equations,
  ...
  \end{align}
Where the \emph{kryptotype} $\kappa(t)$, is a list of a system's molecular concentrations at time $t$ -- or more generally the systems \emph{internal dynamics}. As some system dynamics may not be visible nor relevant to selection, we distinguish the \emph{kryptotype} (as it is hidden) from the \emph{phenotype} -- which is the systems \emph{external dynamics}.
The change in internal molecular concentrations as a function of time $\dot{\kappa}(t)$ is simply a function of the systems current state (or kryptotype), the system architecture $A$ -- such as a gene regulatory network, the environmental input $u(t)$, and $B$ (an $n \times l$ matrix) -- how the system processes its environment. 
\plr{Could say instead of "filters" that e.g. $C_{ij}$ is the amount by which the $j$th input $u_j$ affects the production of the $i$th transcription factor concentration.}
$C$ (an $l \times n$ matrix) filters the dynamics relevant to survival.
...
\plr{You've missed out $\kappa(0)$. Also, you can't have $t$ be both a limit of the integral and the variable being integrated over -- check this and get it right.}
  \begin{align}
    \phi(t) = \int_{0}^{t} C e^{A t} B u(t) dt ,
  \end{align}
where we sometimes refer to $h(t) := Ce^{A t}B$ as the \emph{impulse response} of the system. 

\plr{How about expanding this minor comment into a short paragraph with more interpretation of not just $A$ but also $B$ and $C$? Should say why. 
Include talking through why $A_{ij}>0$ means that $j$ "upregulates" $i$.}
As $A$ specifies gene regulatory network architecture, the $i^\text{th}$ row of $A$ can be interpreted as the \emph{cis}-regulatory element (or promoter) for gene $i$. 
...
\plr{you haven't explained $u(t)$ above. and, isn't it more natural in this set-up to have $B = [1,0]$ than $[1,1]$?}
 ...
 %    \end{align*}
  ...
  \section*{Linear Evolutionary Systems}
\plr{instead title this section "Systems drift for linear systems"?}
...
Despite a symmetry in functionality or phenotype these systems often differ, sometimes substantially, at the molecular level. 
How many different mechanisms have the same function? 
We urge the reader to consider the metaphorical black box.

Gene regulatory networks with identical phenotypes (external dynamics) do not necessarily have identical kryptotypes (internal dynamics). 
...
-- has an identical phenotype up to a change of coordinates. 
\plr{"has an identical phenotype" is not what you mean to say here}
  ...
  \end{align}
\plr{what are we calling these -- not "biological system" -- that's too broad}
Two biological systems, $\mathcal{S} = \{ A, B, C \}$, and $\bar{\mathcal{S}} = \{\bar{A} = VAV^{-1}, \bar{B} = VB, \bar{C} = CV^{-1} \}$, 
have the same phenotype if they are related by a change of coordinates. 
...
\plr{I don't think it makes sense to write things in general as a function of $\tau$,
    since in general the set of "free parameters" is complicated and we don't actually specify it.
    Better, maybe define $\mathcal{V}$ to be the set of all $V$ that preserve $B$ and $C$, then define $A(V)$ instead of $A(\tau)$...}
...
\plr{why is this "evolutionary"? It is the thing that we're thinking about evolution of, but that doesn't make it evolutionary.}
  \begin{align}
    \mathcal{S}(\tau) := \left\{ \begin{array}{ll} \dot{\kappa}(t) &= A(\tau) \kappa(t) + B u(t) \\ \phi(t) &= C \kappa(t) \end{array} \right .
  \end{align}
\plr{again, lacking an object}
That is, all (linear and minimal) mechanisms capable of producing the same phenotype can be realized by a unique choice of $\tau$ in $\mathcal{S}(\tau)$.
...
Since the argument applies more generally, we abstract the model above.
...
so that the fitness of an organism with coefficents $x$ is $\fitx(x) = \fit(\dph(\Phi(x),\optph)/\beta)$,
where $\beta$ controls the scale on which fitness drops.
We will assume that the map $\Phi$ and the distance function $\dph()$ are smooth,
...
The value of $\sigma$ in particular directions in coefficient space 
depends on constraints imposed by selection and 
the covariance between genetic variation between different coefficients -- known as the $G$ matrix \citep{G_matrix}.
...
For instance, linkage will lead to covariance
...
$\gamma$, how fast phenotype changes moving away from $\optx$;
and $\beta$, how fast fitness drops with distance from the optimal phenotype.
(Concretely, $\gamma$ is the derivative of $\dph(\Phi(x+u z), \Phi(x))$ with respect to $u$ for $x \in \optx$ and $z$ perpendicular to the tangent subspace at $x$.)
With these parameters, a typical individual will have a fitness of around $\fit(\sigma_S \gamma / \beta)$.

\paragraph{Hybridization}
\plr{I put F1s and F2s here, but with less explanation than below. Merge somehow.}
By the arguments above, 
over $T$ generations the population mean will move a distance of order $\sqrt{\sigma_N/T}$ along $\optx$.
A $F_1$ hybrid between the two populations
has one haploid genome from each,
so will be distributed as the mean of two randomly chosen haploids from the two populations,
and so will have mean at the midpoint between their means,
and variance equal to $\sigma$.
A $F_2$ hybrid will be homozygous for one parental allele on averge at half of the loci in the genome,
...
\caption{
    ...
    WRITE CAPTION
    \plri{The figure is not quite right now -- should show all types of F2 in purple.  but then it will be messy.  ideas?}
}
\end{figure}

\plri{improve figure by putting labels on from the following}
\plr{I think we can set $\beta=1$ by defining $\fit$ appropriately?}
...
so that for $x_0$ a point nearby on $\optx$,
\begin{align*}
    \Phi(x_0 + x) = \Phi(x_0) + D \Phi(x_0) \cdot x + O(\|\gamma x\|^2),
\end{align*}
and so $d(\Phi(x_0+x), \Phi(x_0)) \approx  \gamma x$.
The mean $F_1$ is the average of the parental means,
and since the first-order terms in the Taylor series vanish,
has phenotype differing from the optimum by a distance of order $(\gamma z)^2$.
\plri{(Proof: let the parental means be $x_1$ and $x_2$; then as $z=x_2-x_1$,
    $d(\Phi((x_1+x_2)/2), \Phi(x_1)) = D \Phi(x_1) \cdot z/2 + O(\gamma z)^2$,
    but expanding about $x_2$ instead we get
    $d(\Phi((x_1+x_2)/2), \Phi(x_2)) = - D \Phi(x_2) \cdot z/2 + O(\gamma \|z\|)^2$.
    Since $\Phi(x_1) = \Phi(x_2)$ and $D \Phi(x_1) - D \Phi(x_2) = O(\|z\|^2)$,
    we see that $D \Phi(x_1) \cdot z = O(\gamma z)^2$.}
The mean $F_2$ is the same,
but is a distance away from the optimal set of order $z$.
In summary, the typical fitness of an individual in the original population is
$\fit_0 = \fit(\sigma_s \gamma / \beta)$;
of an $F_1$ is
$\fit_1 = \fit((\sigma_s \gamma + \gamma z^2) / \beta)$;
and of an $F_2$ is
$\fit_2 = \fit(\gamma z / \beta)$.

If $\fit(x) = e^{-x^2} \approx 1 - x^2$,
then ...
\plri{reparameterize to define $\fit_1$ as the \emph{drop} in fitness?}
...
\plr{what does "equally present" mean?}
 An F1's genetic regulation is a consequence of both of its genomes, where genes and regulatory sequences from both parents are equally present. 
...
Thus we say that a diploid organism's gene network is simply the average of both of its gene network copies; one from each parent. 
Further, each haploid parental gene network copy is formed via meiosis 
-- by swapping independent genes (specifically their \emph{cis}-regulatory modules) randomly. 
Assuming two distinct but genetically homogeneous populations evolving in allopatry meet and form hybrids, 
the first generation hybrids (F1s) gene network dynamics will be determined by the average of the parental haplotypes. 
The second generation hybrids (F2s), however, will be the product of a meiosis between parental haplotypes followed by an averaging of gametes. 
\plr{To agree with the previous section we need to incorporate within-pop variation in this argument.}
...
%\end{align*}
and $F_{2}$ is the second generation hybrid gene network architecture formed by gametes $G(i)$ and $G(j)$. 
\plr{sentences don't begin with "Where", even in math. Also, there isn't "the" $F_2$ - there are many possible.}
Where each $G$ is formed by randomly swapping rows between $A(\tau)$ and $A(\hat{\tau})$, such that the $i$th  gene comes from $A(\tau)$ ($i$ and $j$ are orthogonal vectors, each element $0$ or $1$, and $i \neq j$).
\begin{align}
...
\begin{align}
    \mathcal{F}\left( \widehat{\phi}(t) \right) = \exp \left\{- \frac{1}{\sigma} \int_{0}^{\infty} \left\lVert \phi(t) - \widehat{\phi}(t) \right\rVert^{2} dt  \right\}  .
\end{align}
...
\plr{This part doesn't need a definition - we don't need to discuss what is or isn't a "species".}
...
F1s created by crossing phenotypically equivalent oscillators $A(0)$ and $A(2)$ have a phenotype of $\phi_{F_{1}}(t) = e^{t}$, 
...
\plr{I don't think this is the right place to introduce this?}
These differences tend to increase with time, quadratically ($x^{2}$) in F2s, and quartically ($x^{4}$) in F1s. 
...
\end{alignat*}
\plr{cut this sentence, was already said:}
...
In general terms, if fitness $\mathcal{F}$ is some function $g$ of phenotypic divergence from an optimal scaled by the strength of selection $\beta$,
\begin{align*}
  \mathcal{F} = g \left( \frac{\phi - \phi_{0}}{\beta} \right)
\end{align*}
...
and $F_{1}$ fitness will decline linearly with respect to $\frac{T}{N_{e}}$. ARGUMENT FOR WHY $\sigma^{2} c_{1} \sim \beta$ HERE.
   \begin{align*}
     \Delta \mathcal{F}_{1} = \left( \frac{\Delta \Phi_{1}}{\beta} = \frac{c_{1} \sigma^{2} \frac{T}{N_{e}}}{\beta} \approx \frac{T}{N_{e}} \right)
\end{align*}
...
\begin{align*}
  ...
  \Delta \mathcal{F}_{2} &= \left( \frac{\Delta \Phi_{2}}{\beta} = \frac{c_{2} \sigma \sqrt{\frac{T}{N_{e}}}}{\beta} \approx \sqrt{\frac{T}{N_{e}}} \right)
\end{align*}
...
The phenotype is, 
\plr{this is wrong, fix it}
\begin{align*}
    \phi(t) = \exp \left\{ \kappa(0) + \int_{0}^{t} h(t) u(t) dt \right\} 
\end{align*}
...
\plr{this is a claim we are the first to do something.  best not to make these claims (and we aren't the first to make math models of these)}
Further, we provide, in clear mathematical language, an analytical description of phenomena hitherto only discussed verbally and conceptually 
...
\plr{check original Bateson/DM papers to see if this accords with those defns}
...
\plr{replace "speciate" with "accumulate genetic incompatibilities"}
Because mechanistically distinct yet phenotypically equivalent biological systems can fail to produce viable hybrids, we predict allopatric populations to speciate 
...
\plr{probably shouldn't claim this is "unexplored"}
Although unexplored presently, this phenomena may lead to insights on evolvability and developmental innovation. 
...
\plr{this paragraph isn't very clear}
