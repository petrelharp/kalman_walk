...
Phenotypically equivalent system evolution may not solely be driven by drift.
Network coefficient evolution may be biased due to epistasis and driven by second-order selective effects;
such processes can lead an evolving population's exploration of network space to regions more densely populated by
and more closely connected to high fitness system architectures \citep{neutral_network_physics}. Our results, however, do not rely on the ``thickness''
of the phenotypically equivalent set, but on the set's \emph{openess} under sexual recombination.
...
It is plausible, however, that the speed at which system drift can generate incompatibilities might diminish
in circumstances where selection for robustness is strong enough to constrain a population to a small section of system space.
Likewise, as the speed of system drift relies on segregating genetic variation, any constraints on such variation,
possibly due to epistasis, genetic architecture \citep{hermisson2003epistasis}, adaptive inertia \citep{alvarez2009effects} or weak gene flow could plausibly slow it down.
