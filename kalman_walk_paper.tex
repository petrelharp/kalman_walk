...
\plr{I put F1s and F2s here, and $\Incompat$, but with less explanation than below. Merge somehow.}
...
\begin{align}
    \fit\left( \widehat{\phi}(t) \right) = \exp \left\{- \frac{1}{\sigma} \int_{0}^{\infty} \left\lVert \phi(t) - \widehat{\phi}(t) \right\rVert^{2} dt  \right\}  .
\end{align}
...
$$\begin{aligned}
    C \to C + \epsilon Z P .
\end{aligned}$$
These degrees of freedom depend on $P$, but both through the change of basis $P$
and the form of allowable $X$, $Y$, and $Z$.

This gives us an upper bound on the number of degrees of freedom.
\plri{maybe? depending on if choice of $P$ affects anything.}
