...
\plr{Note: in the \LaTeX source I'm putting in semantic linebreaks, so it's easy to edit and move around phrases and ideas.}
...
\end{tikzpicture}
    \caption{A graphical depiction of the set of all externally equivalent cell cycle control networks, $A(\tau)$. $\tau$ can take any value \plr{sentence fragment}}
    \end{figure}
 % \end{example}
...
\plr{Below we need to separate variation in $A$ from variation in phenotype.}
...
% \jss{added the phenotypic invariance stuff here (above) -- not sure if it should put elsewhere yet}
  ...
  so in a population of effective size $N_e$, with variation in $A$ of order $\sigma$,
  the population mean binding strengths
  will move a distance of order $\sigma/\sqrt{N_e}$ per generation,
  simply because this is the the variance of the mean of a random sample \citep{lande_drift}.
  ...
  (although perhaps complicated by recombination load \citep{recomb_load}).
    \plr{recombination load won't slow it down actually I don't think}
  ...
  \paragraph{Brownian motion} on the set of equivalent networks:
    \plr{revisit.  should this describe motion \emph{on} the set, or motion \emph{near} it, as in writeup with todd?}
