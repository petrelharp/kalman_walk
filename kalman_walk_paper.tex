...
In this paper we use results on mathematical nonidentifiability from \linelabel{r21}
...
\begin{example}[An oscillator]\label{ex:oscillator} \linelabel{r11b}
...
The Kalman decomposition then classifies each direction in kryptotype space\linelabel{r23}
...
both the change of basis used to obtain the decomposition\linelabel{r24}
...
\paragraph{Sexual reproduction and recombination}\linelabel{r26}
...
\paragraph{Haldane's rule}\linelabel{r15b}
...
which are less fit on average than $F_1$s (see Figure \ref{fig:osc_incompat}). \linelabel{r15e}
\revpoint{1}{4}
...
and when the nature of genetic variation produces $F_2$ hybrid breakdown. \revpoint{2}{1}
...
Consistent with empirical observation of hybrid breakdown \linelabel{r16} %(e.g., \citet{plotner2017chlorosis}),\hypertarget{rev1:6} 
...
i.e., \emph{genetic draft} \citep{gillespie2000genetic}.
...
as different alleles fix in different populations. \linelabel{citemani}
...
as the hybrid phenotypes will be displaced from both parental optimums -- that is maladapted to both parental environments.
...
the cumulative impact of system drift across the many different networks an organism relies on may be substantial. \linelabel{rp11}
...
Phenotypically equivalent system evolution is probably not only driven by neutral genetic drift. \linelabel{r14}
...
\paragraph{Geometric models} \linelabel{quantnote}
