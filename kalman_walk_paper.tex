...
with mean $(x+y)/2$ and covariance matrix $N$.
Here, $N$ is the contribution of ``segregation variance''.
...
and covariance matrix $\widetilde \Sigma/2 + N$.
...
$\Sigma U^{-1} \Sigma + (I/2 - N U^{-1}) \Sigma = N$.
...
\begin{align*}
    \begin{bmatrix}
        \Sigma_{11}^2 + (I/2 - N_{11}) \Sigma_{11}
        & \Sigma_{11} \Sigma_{12} + (I/2 - N_{11})\Sigma_{12} \\
        \Sigma_{12}^T \Sigma_{11} + \Sigma_{12}^T/2 - N_{12}^T \Sigma_{11}
        & \Sigma_{22} - N_{12}^T \Sigma_{12} 
    \end{bmatrix}
    &=
    \begin{bmatrix}
        N_{11} & N_{12} \\
    ...
    \end{bmatrix} .
\end{align*}
The first equation, $\Sigma_{11}^2 + (I/2 - N_{11}) \Sigma_{11} = N_{11}$,
can be solved with the quadratic formula: 
$$\Sigma_{11} = (N_{11} - I/2 + Q)/2$$
for any $Q$ that commutes with $N_{11}$
and is a solution to $Q^2 = (N_{11} - I/2)^2 + 4 N_{11}$.
...
\begin{align*}
    \Sigma_{12} 
    &= 
    \left( \Sigma_{11} + I/2 - N_{11} \right)^{-1} N_{12} \\
    \Sigma_{22}
    &=
    N_{12}^T \Sigma_{12} + N_{22} .
\end{align*}
...
computed from $N$ and $U$ as above,
