...
causes a population to explore the set of mechanisms with identical phenotypes.
...
$((A'+A'')/2, (B'+B'')/2, (C'+C'')/2)$.
In general, how parental systems are recombined and inherited
depends on the genetic architecture of the segregating variation.
%\revpoint{2}{6}
For the purposes of examples in this paper
(Example \ref{ex:hybrid_osc} and Figure \ref{fig:hybs}),
we take the simplest assumption that each system coefficient is encoded by a nonrecombining locus,
and that these loci are unlinked.
For instance, if
one genome's coefficients are written with Greek and the other with Latin,
a possible oscillator gamete might be produced as follows:
...
Code for the oscillator examples will be archived at \url{https://doi.org/10.5281/zenodo.5372992}. We will not be archiving data as this paper does not use any. 
